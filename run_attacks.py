...
import gc
import csv
# from experiments.res_parser import *
...
def make_exp_folders(output_dir, save_results, save_best_pert, save_flow, save_pose, save_imgs, make_test_folder=True):
...
def save_pose_list(motions, pose_quat_gt, datastr, pose_dir):
    ...
    if pose_quat_gt is None:
        ...
        plot_traj(results['gt_aligned'], results['est_aligned'], vis=False, savefigname=pose_dir + '/output.png',
...
def test_clean_multi_inputs(args, loader=None):
    """
    Load clean data from args, evaluate clean data on the model and analyze and return results.
    """
    dataset_idx_list = []
    dataset_name_list = []
    traj_name_list = []
    ...
    loader = args.testDataloader if loader is None else loader
    for traj_idx, traj_data in enumerate(loader):
        ...
        with torch.no_grad():
            # model pass on a single clean trajectory:
    ...
    # statistics over criterion results:
...
def test_adv_trajectories(dataloader, model, motions_target_list, attack, pert,
    ...
    """
    TEST: applying Test criterions on the test data.
    The results folder will be filled with calculations from this function.
    """
            ...
            for traj_idx, traj_name in enumerate(traj_name_list):
                for frame_idx, frame_clean_crit in enumerate(traj_clean_crit_list[traj_idx]):
                    data = {'dataset_idx': dataset_idx_list[traj_idx],
                            ...
                            'adv_delta_ratio_' + crit_str: traj_delta_ratio_crit_list[traj_idx][frame_idx]}  # here probably was a bug
...
def extract_target_motions(dataloader):
    """
    Vlad: the original implementation of the project didn't take into account the data split and was based on args.testDataloader which
    was set to the whole dataset. Target motions were extracted from the test_clean_multi_inputs() func.
    To ensure the split we a separate extraction of targets was needed, which this function provides.
    """
    motions_gt_list = []
...
def run_attacks_train(args):
    ...
    # get test dataset clean results for later use in report functions:
...
# Train\Eval\Test split attack
def run_attacks_train_split(args):
    print("Training and testing an adversarial perturbation on the train/eval/test split")
    print("A single universal will be produced with train and eval data, and then tested on the test")
    # attack_obj is created in utils.py compute_attack_args(..) func using a dict, possible attacks: PGD and Const.
    attack = args.attack_obj
    # extraction and clean evaluation of the test data: initially it was the whole available dataset.
    # EXTRACT ALL DATA:
    dataloader = args.wholeDataloader if args.wholeDataloader is not None else args.testDataloader
    dataset_idx_list, dataset_name_list, traj_name_list, traj_indices, \
    motions_gt_list, traj_clean_criterions_list, traj_clean_motions = \
        test_clean_multi_inputs(args, dataloader)

    # EXTRACT CLEAN CRITERIA LIST:
    traj_clean_rms_list, traj_clean_mean_partial_rms_list, \
    traj_clean_target_rms_list, traj_clean_target_mean_partial_rms_list = tuple(traj_clean_criterions_list)
    ...
    eval_output_dir, eval_flowdir, eval_pose_dir, eval_img_dir, eval_adv_img_dir, eval_adv_pert_dir, \
    test_output_dir, test_flowdir, test_pose_dir, test_img_dir, test_adv_img_dir, test_adv_pert_dir = \
        make_exp_folders(args.output_dir, args.save_results, args.save_best_pert, args.save_flow, args.save_pose, args.save_imgs)

    # EXTRACTING TRAIN DATA:
    train_indices = [traj_idx for traj_idx in traj_indices if dataset_idx_list[traj_idx] in args.train_folders]
    ...
    motions_target_train_list = masked_list(motions_gt_list, train_indices)
    ...
    eval_indices = [traj_idx for traj_idx in traj_indices if dataset_idx_list[traj_idx] in args.eval_folders]
    ...
    motions_target_eval_list = masked_list(motions_gt_list, eval_indices)
    ...
    test_indices = [traj_idx for traj_idx in traj_indices if dataset_idx_list[traj_idx] in args.test_folders]
    ...
    motions_target_test_list = masked_list(motions_gt_list, test_indices)
    ...
    """
    peturb() gets data_loader=testDataLoader, while eval_data_loader=None.
    which means, eval_data_loader will be set to data_loader.
    """
    # TRAIN + EVAL after each epoch:
    best_pert, clean_loss_list, all_loss_list, all_best_loss_list = \
        attack.perturb(data_loader=args.trainDataloader, y_list=motions_target_train_list,
                       eval_data_loader=args.evalDataloader, eval_y_list=motions_target_eval_list, eps=args.eps, device=args.device)
    ...
    # TEST ON TRAIN:
    traj_adv_train_criterions_list = \
        test_adv_trajectories(args.trainDataloader, args.model, motions_target_train_list, attack, best_pert,
                              args.criterions, args.window_size,
                              args.save_imgs, args.save_flow, args.save_pose,
                              train_adv_img_dir, train_adv_pert_dir, train_flowdir, train_pose_dir,
                              device=args.device)
    ...
    # TEST ON EVAL:
    traj_adv_eval_criterions_list = \
        test_adv_trajectories(args.evalDataloader, args.model, motions_target_eval_list, attack, best_pert,
                              args.criterions, args.window_size,
                              args.save_imgs, args.save_flow, args.save_pose,
                              eval_adv_img_dir, eval_adv_pert_dir, eval_flowdir, eval_pose_dir,
                              device=args.device)
    ...
    # TEST ON TEST:
    traj_adv_test_criterions_list = \
        test_adv_trajectories(args.testDataloader, args.model, motions_target_test_list, attack, best_pert,
                              args.criterions, args.window_size,
                              args.save_imgs, args.save_flow, args.save_pose,
                              test_adv_img_dir, test_adv_pert_dir, test_flowdir, test_pose_dir,
                              device=args.device)
    ...
    # TEST ON ALL DATA:-------------------------------------------------------------------------------------
    if args.wholeDataloader is not None:
        traj_adv_whole_criterions_list = \
            test_adv_trajectories(args.wholeDataloader, args.model, motions_gt_list, attack, best_pert,
                                  args.criterions, args.window_size,
                                  args.save_imgs, args.save_flow, args.save_pose,
                                  args.adv_img_dir, args.adv_pert_dir, args.flowdir, args.pose_dir,
                                  device=args.device)

        traj_adv_whole_rms_list, traj_adv_whole_mean_partial_rms_list, \
        traj_adv_whole_target_rms_list, traj_adv_whole_target_mean_partial_rms_list = traj_adv_whole_criterions_list

        # REPORT ALL DATA CRITERIA:----------------------------------------------------------------------
        report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                             traj_clean_target_mean_partial_rms_list, traj_adv_whole_target_mean_partial_rms_list,
                             args.save_csv, args.output_dir, crit_str="target_mean_partial_rms",
                             experiment_name='all')

        report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                             traj_clean_target_rms_list, traj_adv_whole_target_rms_list,
                             args.save_csv, args.output_dir, crit_str="target_rms",
                             experiment_name='all')

        report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                             traj_clean_mean_partial_rms_list, traj_adv_whole_mean_partial_rms_list,
                             args.save_csv, args.output_dir, crit_str="mean_partial_rms",
                             experiment_name='all')

        report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                             traj_clean_rms_list, traj_adv_whole_rms_list,
                             args.save_csv, args.output_dir, crit_str="rms",
                             experiment_name='all')

    # REPORT TRAIN DATA CRITERIA:--------------------------------------------------------------------------------------
    ...
    # REPORT TEST DATA CRITERIA IN TEST FOLDER:-------------------------------------------------------------------------------------
...
def test_clean(args):
    # seems like run_attacks_train(args) doesn't support test mode only, needs to be checked.
    # possibly we don't even need the test mode.
...
def main():
    ...
    # print(args)
    if args.parse_res:
        print('Parsing results, no training')
        # results = parse_results()
        # print_results(results)
        # return results
        return []
    elif args.attack is None:  # CLI parser sets default attack as 'none' str.
        # TEST:
        return test_clean(args)
    elif args.attack_oos:
        return run_attacks_out_of_sample_test(args)
    elif len(args.split_array) == 5 or args.eval_folder != -1:
        # Train/Eval/Test case:
        return run_attacks_train_split(args)
    # Default case:
    return run_attacks_train(args)
