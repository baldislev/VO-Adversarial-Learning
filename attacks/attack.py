...
class Attack:
    def __init__(self, model, criterion, test_criterion, norm, data_shape,
                 ...
                 pert_padding=(0, 0), stochastic=False, minibatch_size=1):
        self.model = model
        ...
        # Vlad: those two lines seem like a bug:
        ...
        if sample_window_size is not None:
            # Split trajectory into several non disjoint subtrajectories.
    ...
    def test_pert_sample(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
        ...
        # USED in test_pert which is not used
        mask1, mask2, perspective1, perspective2 = self.prep_data(mask, perspective)
        pert = pert.detach()
    ...
    def attack_eval(self, pert, data_shape, eval_data_loader, eval_y_list, device):
        with torch.no_grad():
            ...
            # Evaluation over the whole train data:
            for data_idx, data in enumerate(eval_data_loader):
                # Extract trajectory and prepare input:
                dataset_idx, dataset_name, traj_name, traj_len, \
                img1_I0, img2_I0, intrinsic_I0, \
                img1_I1, img2_I1, intrinsic_I1, \
                img1_delta, img2_delta, \
                ...
                # Apply the model on the given perturbation. Will use perturb_model_split() if window mode is set:
                img1_adv, img2_adv, output_adv = self.perturb_model(pert_expand, img1_I0, img2_I0,
                                                                    ...
                                                                    perspective2,
                                                                    device)
                # evaluate loss over a criterion that is different from the training loss function:
                ...
                # current_step = rundata.current_step
                # writer.add_scalar("loss/item1",loss[0] ,current_step)
                # writer.add_scalar("loss/item2", loss[1],current_step)
                # writer.add_scalar("loss/item3", loss[2],current_step)
                # writer.add_scalar("loss/item4", loss[3],current_step)
                # writer.add_scalar("loss/item5", loss[4],current_step)
                # writer.add_scalar("loss/item6", loss[5],current_step)
                # writer.add_scalar("loss/item7", loss[6],current_step)
                # writer.add_scalar("loss/item8", loss[7],current_step)
                # writer.add_scalar("loss/sum", loss_sum,current_step)
                rundata.current_step += 1
    ...
    def gradient_ascent_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
        ...
        """
        Implements a single epoch pass over the train data:
        """
        pert_expand = pert.expand(data_shape[0], -1, -1, -1).to(device)
        grad_tot = torch.zeros_like(pert, requires_grad=False)
        grad_batch = torch.zeros_like(pert, requires_grad=False)
        # FULL pass on all data:
        for data_idx, data in enumerate(data_loader):
            rundata.traj_idx = data_idx
            # Traj Extraction:
            dataset_idx, dataset_name, traj_name, traj_len, \
            img1_I0, img2_I0, intrinsic_I0, \
            img1_I1, img2_I1, intrinsic_I1, \
            ...
            mask1, mask2, perspective1, perspective2 = self.prep_data(mask, perspective)
            # calls calc_sample_grad_split or calc_sample_grad_single depending if a window option was required.
            ...
            with torch.no_grad():
                if self.stochastic:
                    if self.minibatch_size == 1:  # pure stochastic:
                        pert = self.gradient_ascent_perturb_step(pert, grad, a_abs, multiplier, eps)
                    elif data_idx % (self.minibatch_size - 1) == 0:  # minibatch:
                        pert = self.gradient_ascent_perturb_step(pert, grad_batch, a_abs, multiplier, eps)
                        grad_batch = torch.zeros_like(pert, requires_grad=False)
                    else:
                        grad_batch += grad
                else:
        ...
        if not self.stochastic:  # Classical GD:
            with torch.no_grad():
                pert = self.gradient_ascent_perturb_step(pert, grad_tot, a_abs, multiplier, eps)
