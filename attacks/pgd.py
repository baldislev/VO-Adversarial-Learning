...
class PGD(Attack):
    def __init__(
        ...
        # -------------------------------------------------------------------
        # beta: decay factor
        # t_decay: decay the step size for the attack every t_decay iteration
        self.beta = beta
        self.t_decay = t_decay if t_decay is not None else n_iter
        # -------------------------------------------------------------------
    ...
    def calc_sample_grad_single(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
        ...
        writer.add_scalar("loss/train_loss", loss_sum, rundata.epoch)
        rundata.lossstep += 1
        # grad step
    ...
    def perturb(self, data_loader, y_list, eps, targeted=False, device=None, eval_data_loader=None, eval_y_list=None):
        ...
        for rest in tqdm(range(self.n_restarts)):
            ...
            for k in tqdm(range(self.n_iter)):
                ...
                rundata.epoch = k
                if self.t_decay and self.beta and (k + 1) % self.t_decay == 0:
                    a_abs = a_abs * self.beta
                ...
                with torch.no_grad():
                    ...
                    eval_loss_tot, eval_loss_list = self.attack_eval(pert, data_shape, eval_data_loader, eval_y_list,
                                                                     device)
                    writer.add_scalar("loss/eval_loss_tot", eval_loss_tot, rundata.epoch)
...
class MomentumPDG(PGD):
    def __init__(
            ...
            init_pert_transform=None,
            momentum=None):
        super(MomentumPDG, self).__init__(model, criterion, test_criterion, data_shape, norm, n_iter, n_restarts, alpha,
                                          beta, t_decay, rand_init, sample_window_size, sample_window_stride,
                                          pert_padding, stochastic, minibatch_size, init_pert_path, init_pert_transform)
...
class RMSPropPGD(PGD):
    def __init__(
            ...
            decay=0.9,
            epsilon=1e-8):
        super(RMSPropPGD, self).__init__(model, criterion, test_criterion, data_shape, norm, n_iter, n_restarts, alpha,
                                         beta, t_decay, rand_init, sample_window_size, sample_window_stride,
                                         pert_padding, stochastic, minibatch_size, init_pert_path, init_pert_transform)
        self.decay = decay
        self.epsilon = epsilon
        self.prev_grad_sq = None
        self.prev_sq_average = None

    def gradient_ascent_perturb_step(self, pert, grad, a_abs, multiplier, eps):
        grad_sq = grad * grad
        if self.prev_grad_sq is not None:
            sq_average = self.decay * self.prev_sq_average + (1 - self.decay) * grad_sq
        else:
            sq_average = (1 - self.decay) * grad_sq
        self.prev_sq_average = sq_average
        grad = self.normalize_grad(grad)
        pert += multiplier * grad * (a_abs / (torch.sqrt(sq_average + self.epsilon)))
        pert = self.project(pert, eps)
        return pert
