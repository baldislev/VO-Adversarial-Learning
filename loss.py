...
def aggregate_loss(item):
    if isinstance(item, int) or isinstance(item, float) or item.dim() == 0:
        return item
    else:
        return item.sum(dim=0)
...
class VOCriterion:
    def __init__(self, t_crit='rms', rot_crit='none', flow_crit='none', target_t_crit='none',
        ...
        # elif t_crit == 'gil':
        #     self.calc_t_crit = self.calc_rms_traj
        # elif t_crit == 'gil_sub':
        #     self.calc_t_crit = self.calc_rms_traj_sub
        else:  # rms
            self.calc_t_crit = self.calc_cumul_poses_t
        # ------------------------------------------------------------------------------
        self.rot_crit = rot_crit
        if rot_crit == 'quat_product':
            self.calc_rot_crit = self.calc_rot_quat_product
        # elif rot_crit == 'gil_rot':
        #     self.calc_rot_crit = self.calc_angle_sim_target
        else:
        ...
        if flow_crit == 'mse':
            ...
            self.calc_flow_crit = CalcCriterion(L1Loss)
        else:
    ...
    def apply(self, model_output, scale, motions_gt, target_pose, flow_clean=None):
        ...
        flow_crit = torch.zeros(motions.shape[0] + 1,
                                device=motions.device, dtype=motions.dtype)
        # TODO: those two need to be implemented: DONE
        # if target_pose is not None and self.rot_crit == 'gil_rot':
        #     rot_crit[1:] = self.calc_rot_crit(motions, motions_gt, target_pose)
        # else:
        rot_crit[1:] = scale * self.calc_rot_crit(motions, motions_gt)
        flow_crit[1:] = scale * self.calc_flow_crit(flow, flow_clean)

        if rundata.run_now:
            writer.add_scalar("loss/t_factor_*_t_crit", aggregate_loss(self.t_factor * t_crit), rundata.epoch)
            writer.add_scalar("loss/target_t_factor * target_t_crit", aggregate_loss(self.target_t_factor * target_t_crit), rundata.epoch)
            writer.add_scalar("loss/flow_factor * flow_crit", aggregate_loss(self.flow_factor * flow_crit), rundata.epoch)
            writer.add_scalar("loss/rot_factor * rot_crit", aggregate_loss(self.rot_factor * rot_crit), rundata.epoch)

            if rundata.epoch == 0 and rundata.lossstep == 0:
                writer.add_scalar("hparam/t_factor", self.t_factor, rundata.epoch)
                writer.add_scalar("hparam/target_t_factor", self.target_t_factor, rundata.epoch)
                writer.add_scalar("hparam/flow_factor", self.flow_factor, rundata.epoch)
                writer.add_scalar("hparam/rot_factor", self.rot_factor, rundata.epoch)

        # print(f'EPOCH={rundata.epoch} \n'
        #       f't_crit_dim={type(t_crit).__name__}, t_crit={self.t_factor * t_crit} \n'
        #       f'target_t_crit_dim={type(target_t_crit).__name__}, target_t_crit={self.target_t_factor * target_t_crit} \n'
        #       f'flow_crit_dim={type(flow_crit).__name__}, flow_crit={self.flow_factor * flow_crit} \n'
        #       f'rot_crit_dim={type(rot_crit).__name__}, rot_crit={self.rot_factor * rot_crit}')
    ...
    # Gil ------------------------------------------------------------------------------------------------------------------
    def calc_rms_traj(self, motions, motions_gt, target_pose):
        rel_poses = self.rtvec_to_pose(motions)
        rel_poses_gt = self.rtvec_to_pose(motions_gt)
        loc = self.cumulative_poses(self.rtvec_to_pose(motions))
        gt_loc = self.cumulative_poses(self.rtvec_to_pose(motions_gt))
        diff = torch.sum((loc[:, :3, 3] - gt_loc[:, :3, 3]) ** 2) / motions.shape[0]
        return diff, torch.zeros_like(diff)

    def calc_rms_traj_sub(self, motions, motions_gt, target_pose):
        total = 0
        for len in range(2, motions.shape[0] + 1):
            for start in range(motions.shape[0] + 1 - len):
                total += 1
        # total = sum(map(lambda x: ((motions.shape[0] - (x - 1)) * x, range(2, motions.shape[0]+1))))
        final = torch.zeros(total, 1)
        ind = 0
        for len in range(2, motions.shape[0] + 1):
            for start in range(motions.shape[0] + 1 - len):
                rel_poses = self.rtvec_to_pose(motions[start:start + len, :])
                rel_poses_gt = self.rtvec_to_pose(motions_gt[start:start + len, :])
                loc = self.cumulative_poses(rel_poses)
                gt_loc = self.cumulative_poses(rel_poses_gt)
                diff = torch.sum((loc[:, :3, 3] - gt_loc[:, :3, 3]) ** 2) / len
                final[ind] = diff
                ind += 1
        loss = torch.sum(final) / total
        return loss, torch.zeros_like(diff)

    def calc_angle_sim_target(self, motions, motions_gt, target_pose):
        loc = self.cumulative_poses(self.rtvec_to_pose(motions))
        vector = loc[1:, :3, :3] @ torch.Tensor([[1, 0, 0]]).T.cuda()
        diffs = torch.nn.functional.cosine_similarity(vector, target_pose, 1)
        diff = torch.sum(diffs) / motions.shape[0]
        return diff
